...
for epoch in tqdm(range(EPOCHS)):
    ...
    selector = SelectKBest(score_func=f_classif, k=10)  # Select top 10 features using ANOVA F-value
    train_fusion_selected = selector.fit_transform(train_fusion, train_label)


    if bst is None:
        bst = lgb.train(params, lgb.Dataset(train_fusion_selected, train_label), 100)
    else:
        bst = lgb.train(params, lgb.Dataset(train_fusion_selected, train_label), 100, init_model=bst)

    # model prediction, predict the probability of each sample
    train_ypred = bst.predict(train_fusion_selected)

    # Calculate train_acc
    train_ypred_fin = np.where(train_ypred > 0.5, 1, 0)
    train_acc = np.mean(train_ypred_fin == train_label)

    # Calculate train_auc
    train_auc = roc_auc_score(train_label, train_ypred)
    ...
    test_fusion = np.concatenate((test_data, test_conv), axis=1)   # 两个特征拼接

    test_fusion_selected = selector.transform(test_fusion)

    # model prediction, predict the probability of each sample
    test_ypred = bst.predict(test_fusion_selected)

    # Calculate test_acc
    test_ypred_fin = np.where(test_ypred > 0.5, 1, 0)
    test_acc = np.mean(test_ypred_fin == test_label)

    # Calculate test_auc
    test_auc = roc_auc_score(test_label, test_ypred)

    # Calculate f1_score
    test_f1 = f1_score(test_label, test_ypred_fin)

    # Calculate recall
    test_recall = recall_score(test_label, test_ypred_fin)

    csv_log_out.write("{},{},{},{},{},{},{}\n".format(epoch, train_acc, train_auc, test_acc, test_auc, test_f1, test_recall))
    csv_log_out.flush()
