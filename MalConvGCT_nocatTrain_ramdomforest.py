...
for epoch in tqdm(range(EPOCHS)):
    ...
    train_fusion = np.concatenate((train_data, train_conv), axis=1)   # 两个特征拼接

    if bst is None:
        bst = RandomForestClassifier(n_estimators=100, random_state=0)
        bst.fit(train_fusion, train_label)
    else:
        bst.estimators_ += RandomForestClassifier(n_estimators=100, random_state=0).fit(train_fusion, train_label).estimators_

    # model prediction, predict the probability of each sample
    train_ypred = bst.predict_proba(train_fusion)
    train_ypred_max = np.max(train_ypred, axis=1)

    # Calculate train_acc
    train_ypred_fin = np.argmax(train_ypred, axis=1)
    train_acc = np.mean(train_ypred_fin == train_label)

    # Calculate train_auc
    train_auc = roc_auc_score(train_label, train_ypred_max)
    ...
    # model prediction, predict the probability of each sample
    test_ypred = bst.predict_proba(test_fusion)
    test_ypred_max = np.max(test_ypred, axis=1)

    # Calculate test_acc
    test_ypred_fin = np.argmax(test_ypred, axis=1)
    test_acc = np.mean(test_ypred_fin == test_label)

    # Calculate test_auc
    test_auc = roc_auc_score(test_label, test_ypred_max)
