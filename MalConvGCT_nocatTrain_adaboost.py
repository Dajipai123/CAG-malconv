...
pretrained_model_path = "/home/fhh/ember-master/MalConv2-main/least_f1.checkpoint"
...
def get_intermediate_activations(model, data_loader):
    ...
    return outputs,penultimate_activations, conv_activations, labels

def select_feature(data, label, k):
    # 选择特征
    select = SelectKBest(f_classif, k=k)
    select.fit(data, label)
...
for epoch in tqdm(range(EPOCHS)):
    ...
    # train process......
    outputs,penultimate_activations, conv_activations, labels = get_intermediate_activations(model, train_loader)
    ...
    train_data = np.concatenate(penultimate_activations, axis=0)
    train_label = np.concatenate(labels, axis=0)
    train_conv = np.concatenate(conv_activations, axis=0)
    train_fusion = np.concatenate((train_data, train_conv), axis=1)   # Concatenate two features

    # Perform feature selection
    # train_select_data = select_feature(train_fusion, train_label, 32)
    train_select_data = pca(train_fusion, 32)

    # test process......
    outputs, penultimate_activations, conv_activations, labels = get_intermediate_activations(model, test_loader)
    # Move the data to CPU
    outputs = outputs.cpu()
    penultimate_activations = [activations.cpu() for activations in penultimate_activations]
    conv_activations = [activations.cpu() for activations in conv_activations]
    ...
    test_fusion = np.concatenate((test_data, test_conv), axis=1)   # Concatenate two features

    # Perform feature selection
    # test_select_data = select_feature(test_fusion, test_label, 32)
    test_select_data = pca(test_fusion, 32)

    if epoch == 0:
        x_train = train_select_data
        x_test = test_select_data
    else:
        x_train = np.concatenate((x_train, train_select_data), axis=1)
        x_test = np.concatenate((x_test, test_select_data), axis=1)
...
# 训练adaboost模型
bst =  AdaBoostClassifier(n_estimators=100, learning_rate=0.1)
bst.fit(x_train, y_train)

# 预测
test_TP_probability = bst.predict_proba(x_test)[:, 1]
...
# 计算test_auc
test_TP_auc = roc_auc_score(y_test, test_TP_probability)

# 计算test_f1_score
test_TP_f1_score = f1_score(y_test, test_pred)

# 计算test_recall
test_TP_recall = recall_score(y_test, test_pred)

print(f"test_acc:{test_acc}, test_auc:{test_TP_auc}, test_f1_score:{test_TP_f1_score}, test_recall:{test_TP_recall}")

csv_log_out = open("/home/fhh/ember-master/MalConv2-main/adaboost_log.csv", "a")
headers = ['test_acc', 'test_auc', 'test_f1_score', 'test_recall']
csv_log_out.write(",".join(headers) + "\n")
csv_log_out.write("{},{},{},{}\n".format(test_acc, test_TP_auc, test_TP_f1_score, test_TP_recall))
csv_log_out.flush()

# 保存模型
# bst.save_model('/home/fhh/ember-master/MalConv2-main/adaboost_model/adaboost_model.txt')
