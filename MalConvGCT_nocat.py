...
class MalConvGCT(LowMemConvBase):
    # 注意！这里的outsize要改回2
    def __init__(self, out_size=2, channels=128, window_size=512, stride=512, layers=1, embd_size=8, log_stride=None, low_mem=True, device=None):
        ...
        self.context_net = MalConvML(out_size=channels, channels=channels, window_size=window_size, stride=stride, layers=layers, embd_size=embd_size, device=device)
        ...
        #     nn.LeakyReLU(),
        #     nn.Linear(channels // 4, out_size)
        # )

        self.feed_forward = nn.Linear(channels, channels)
        self.classifier = nn.Linear(channels, out_size)
    ...
    def forward(self, x):
        ...
        penult = x = F.leaky_relu(self.feed_forward( x ))
        x = self.classifier(x)
        
        # penult's and post_conv's shape is ([128,128])
